namespace Kheper.Web
{
    ...
    public class PreemptiveAutowiring : IRegistrationSource
    {
        ...
        {
            IEnumerable<IComponentRegistration> items = registrationAccessor(service);
            if (items.Any())
            {
                return items;
            }
            Debug.Print("Autowiring: " + service);
            if (service.Description.EndsWith("Repository"))
            {
                Debug.Print("Found");
            }

            IEnumerable<IComponentRegistration> registrations = this.container.ComponentRegistry.RegistrationsFor(service);
